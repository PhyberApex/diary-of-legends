...
//					"db\\test.db");
			new File("C:\\update").mkdir();
			File imgFolder = new File("C:\\update\\img");
			imgFolder.mkdir();
			BufferedWriter bw;

			bw = new BufferedWriter(new FileWriter(new File(
					"C:\\update\\update.dolup"), false));
			List<Champion> champs = ChampionUtil.getAllChampions();
			List<Item> items = ItemUtil.getAllItems();
			for (Champion champ : champs) {
				File img = new File(imgFolder.getAbsolutePath()+"\\"+champ.getName()+".png");
				img.createNewFile();
				FileOutputStream fos = new FileOutputStream(img.getAbsolutePath());
				fos.write(champ.getIconRaw());
				fos.close();
				String newLine = "[NEW][CHAMPION][";
				newLine += champ.getName()+",";
				newLine += "img\\"+champ.getName()+".png]";
				bw.write(newLine);
				bw.newLine();
			}
			for (Item item : items) {
				FileOutputStream fos = new FileOutputStream(imgFolder.getAbsolutePath()+"\\"+item.getName()+".png");
				fos.write(item.getIconRaw());
				fos.close();
				String newLine = "[NEW][ITEM][";
				newLine += item.getName()+",";
				newLine += item.getPrice()+",";
				newLine += "img\\"+item.getName()+".png]";
				bw.write(newLine);
				bw.newLine();
			}
			bw.close();
